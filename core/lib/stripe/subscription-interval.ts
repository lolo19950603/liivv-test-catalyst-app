...
export function getDefaultSubscriptionBillingInterval(): SubscriptionBillingInterval {
  const [first] = parseIntervalList(DEFAULT_STRIPE_SUBSCRIPTION_INTERVAL_OPTIONS);

  return first ?? { interval: 'month', intervalCount: 1 };
}
...
export function resolveSelectedSubscriptionBillingInterval(
  ...
  return allowedIntervals[0] ?? getDefaultSubscriptionBillingInterval();
}
